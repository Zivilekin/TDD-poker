...
exports.stateErrors = {
    ...
    STATE_VARIABLE_NAME_CLASH: function (name) { return "state \"" + name + "\" definition cannot begin with \"--\""; }
};
...
function processPseudoStates(value, decl, diagnostics) {
    ...
    statesSplitByComma.forEach(function (workingState) {
        ...
        if (stateDefinition.value.trim().startsWith('--')) {
            diagnostics.error(decl, exports.stateErrors.STATE_VARIABLE_NAME_CLASH(stateDefinition.value), { word: stateDefinition.value });
        }
...
function resolveStateType(stateDefinition, mappedStates, stateDefault, diagnostics, decl) {
    if (stateDefinition.type === 'function' && stateDefinition.nodes.length === 0) {
        resolveBooleanState(mappedStates, stateDefinition);
        diagnostics.warn(decl, exports.stateErrors.NO_STATE_TYPE_GIVEN(stateDefinition.value), { word: decl.value });
        return;
    }
        ...
        if (container.type !== 'atrule') {
            ...
            if (sRule.selectorAst.nodes && sRule.selectorAst.nodes.length === 1) {
                ...
                if (selectorChunk.length === 1 && selectorChunk[0].type === 'class') {
                    ...
                    var states = classMeta[stylable_value_parsers_2.valueMapping.states];
                    if (classMeta && classMeta._kind === 'class' && states) {
                        for (var stateName in states) { // TODO: Sort out types
...
function validateStateArgument(stateAst, meta, value, resolver, diagnostics, rule, validateDefinition, validateValue) {
    ...
    var paramType = stateAst.type;
    var validator = state_validators_1.systemValidators[paramType];
    try {
        if (resolvedValidations.res || validateDefinition) {
            var errors = validator.validate(resolvedValidations.res, stateAst.arguments, resolveParam.bind(null, meta, resolver, diagnostics, rule), !!validateDefinition, validateValue).errors;
...
function transformPseudoStateSelector(meta, node, name, symbol, origin, originSymbol, resolver, diagnostics, rule) {
    ...
    while (current && currentSymbol) {
        if (currentSymbol._kind === 'class' || currentSymbol._kind === 'element') {
            ...
            if (states && states.hasOwnProperty(name)) {
                ...
                var next = resolver.resolve(alias);
                if (next && next.meta) {
                    currentSymbol = next.symbol;
                    current = next.meta;
                }
    ...
    if (!found && rule) {
        if (native_reserved_lists_1.nativePseudoClasses.indexOf(name) === -1 && !isVendorPrefixed(name)) {
...
function resolveStateValue(meta, resolver, diagnostics, rule, node, stateDef, name, namespace) {
    ...
    var strippedParam = utils_1.stripQuotation(actualParam);
