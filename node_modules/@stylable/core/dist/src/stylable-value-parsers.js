...
exports.valueParserWarnings = {
    VALUE_CANNOT_BE_STRING: function () { return 'value can not be a string (remove quotes?)'; },
    CSS_MIXIN_FORCE_NAMED_PARAMS: function () { return 'CSS mixins must use named parameters (e.g. "func(name value, [name value, ...])")'; }
};
...
var strategies = {
    named: function (node, reportWarning) {
        var named = {};
        getNamedArgs(node).forEach(function (mixinArgsGroup) {
            var argsDivider = mixinArgsGroup[1];
            if (mixinArgsGroup.length < 3 || (argsDivider && argsDivider.type !== 'space')) {
                if (reportWarning) {
                    var argValue = mixinArgsGroup[0];
                    reportWarning(exports.valueParserWarnings.CSS_MIXIN_FORCE_NAMED_PARAMS(), { word: argValue.value });
                }
                return;
            }
