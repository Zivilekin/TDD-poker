...
function isRootValid(ast, rootName) {
    var isValid = true;
    traverseNode(ast, function (node, index, nodes) {
        if (node.type === 'nested-pseudo-class') {
            return true;
        }
        if (node.type === 'class' && node.name === rootName) {
            var isLastScopeGlobal = false;
...
function createWarningRule(extendedNode, scopedExtendedNode, extendedFile, extendingNode, scopedExtendingNode, extendingFile) {
    ...
    var message = "\"class extending component '." + extendingNode + " => " + scopedExtendingNode + "' in stylesheet '" + extendingFile + "' was set on a node that does not extend '." + extendedNode + " => " + scopedExtendedNode + "' from stylesheet '" + extendedFile + "'\" !important";
    return postcss.rule({
