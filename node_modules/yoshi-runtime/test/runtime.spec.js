...
const {replaceSassWebpackImports} = require('../utils');
...
describe('CSS modules runtime', () => {
  ...
  describe('replaceSassWebpackImports', () => {
    it('should replace webpack format for sass imports to the original', () => {
      const webpackSassImports = `
        @import '~package/my/file'
        @import '~package2/my/other-file'
      `;
      const cwd = process.cwd();
      const originalSassImport = `
        @import '${path.join(cwd, 'node_modules/package/my/file')}'
        @import '${path.join(cwd, 'node_modules/package2/my/other-file')}'
      `;
      const result = replaceSassWebpackImports(webpackSassImports);

      expect(result).to.equal(originalSassImport);
    });
  });
});
